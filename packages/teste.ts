import { ForeignFunction, Library } from './ffiLibraries';
...
const versao = new Promise<string | null>((resolve, reject) => {
    try {
       const versao = ffiLibrary.VersaoLib();
       resolve(versao);
    } catch (error) {
        reject(error);
    }
});
const versao2 = new Promise((resolve, reject) => {
    try {
